...
class RedPandaNodeLauncher {
  ...
  RedPandaNodeLauncher({
    required this.port,
    this.seeds = const [],
  }) : _workingDir = Directory.systemTemp.createTempSync('redpanda_node_$port').path;
  ...
  Future<void> start() async {
    ...
    final jarPath = p.join(projectRoot, 'references', 'redPandaj', 'target', 'redpanda.jar');

    if (!File(jarPath).existsSync()) {
      throw Exception('redpanda.jar not found at $jarPath. Did you run "mvn package"?');
    }
  ...
  String _findProjectRoot() {
    ...
    while (true) {
      if (File(p.join(dir.path, 'references', 'redPandaj', 'pom.xml')).existsSync()) {
        return dir.path;
      }
      final parent = dir.parent;
      if (parent.path == dir.path) {
        throw Exception('Could not find project root containing references/redPandaj');
      }
      dir = parent;
    }
  }
